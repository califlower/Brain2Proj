...
def plot_2d_input_weights():
    ...
    fig = b2.figure(fig_num, figsize = (48, 48))
...
start = time.time()
testing = get_labeled_data('testing/processed.csv')
end = time.time()
...
if num_examples <= 2043:
    update_interval = num_examples
    weight_update_interval = 20
else:
    update_interval = 2043
...
while j < (int(num_examples)):
    if test_mode:
        if use_testing_set:
            spike_rates = testing['x'][j%2043,:,:].reshape((n_input)) / 8. *  input_intensity
        else:
            spike_rates = training['x'][j%3499,:,:].reshape((n_input)) / 8. *  input_intensity
    else:
        normalize_weights()
        spike_rates = training['x'][j%3499,:,:].reshape((n_input)) / 8. *  input_intensity
    ...
    if j % weight_update_interval == 0 and not test_mode:
        pass # update_2d_input_weights(input_weight_monitor, fig_weights)
    ...
    if np.sum(current_spike_count) < 5:
        ...
        if test_mode and use_testing_set:
            input_numbers[j] = testing['y'][j%2043][0]
        else:
            input_numbers[j] = training['y'][j%3499][0]
